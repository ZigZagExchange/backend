...
describe("fillOrderExactOutput", function () {
    ...
    it("Should revert with 'taker order not enough balance' ", async function () {
        ...
        await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutput(Object.values(makerOrder), signedLeftMessage, fillAmount, true)).to.be.revertedWith('taker order not enough balance');
    });
  ...
  it("Should revert with 'taker order not enough allowance' ", async function () {
    ...
    it("Should revert when maker order is already filled", async function () {
        ...
        await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutput(Object.values(makerOrder), signedLeftMessage, fillAmount, true)).to.be.revertedWith('order is filled');
    });
    ...
    it("Should revert when maker order is expired", async function () {
        ...
        const fillAmount = ethers.utils.parseEther("1");
        const signedLeftMessage = await signOrder(TESTRPC_PRIVATE_KEYS_STRINGS[0], makerOrder, exchangeContract.address)
        await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutput(Object.values(makerOrder), signedLeftMessage, fillAmount, true)).to.be.revertedWith('order expired');
    });
    ...
    it("should fail when filled twice", async function () {
        ...
        const fillAmount = ethers.utils.parseEther("100");
        const tx = await exchangeContract.connect(wallets[1]).fillOrderExactOutput(Object.values(makerOrder), signedLeftMessage, fillAmount, true)
        await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutput(Object.values(makerOrder), signedLeftMessage, fillAmount, true))
    ...
    it("should fail without fillAvailable when over-ordering", async function () {
        ...
        await expect(tx2).to.be.revertedWith('buyAmount exceeds available size');
    });

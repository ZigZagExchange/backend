...
describe('Order Matching', () => {
  ...
  beforeEach(async function () {
    ...
    await tokenA
      .connect(wallets[0])
      .approve(exchangeContract.address, ethers.utils.parseEther('10000'))
    await tokenB
      .connect(wallets[1])
      .approve(exchangeContract.address, ethers.utils.parseEther('10000'))
  ...
  it("Should revert with 'orders not crossed' ", async () => {
    const makerOrder = {
    ...
    const signedRightMessage = await signOrder(
    ...
    await expect(
      exchangeContract
        ...
        .matchOrders(
          ...
          Object.values(takerOrder),
          signedLeftMessage,
          signedRightMessage
        )
    ).to.be.revertedWith('orders not crossed')
  })
  ...
  it("Should revert with 'taker order not enough balance for fee' ", async () => {
    const makerOrder = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
    ...
    ).to.be.revertedWith('taker order not enough balance for fee')
  })

  it("Should revert with 'maker order not enough balance for fee' ", async () => {
    ...
    ).to.be.revertedWith('maker order not enough balance for fee')
  })
  ...
  it('Should revert with mismatched tokens from mismatching maker and taker tokens', async () => {
    const makerOrder = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
    ...
    ).to.be.revertedWith('mismatched tokens')
  })

  it('Should revert when taker order is already filled', async () => {
    const makerOrder = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
  ...
  it('Should revert when taker order is canceled', async () => {
    const makerOrder = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
  ...
  it('Should revert when order time is expired', async () => {
    const makerOrder = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
  ...
  it('feeRecipient should take partial Taker Fee', async () => {
    const makerOrder = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
  ...
  it('same price should match', async () => {
    // tokenA = ETH
    // tokenB = USDC
    const makerOrder = {
      user: wallets[0].address,
  ...
  it('should fill at maker price - market buy into ask - fill maker fully', async () => {
    // tokenA = ETH
    // tokenB = USDC
  ...
  it('should fill at maker price - market buy into ask - fill maker partially', async () => {
    // tokenA = ETH
    // tokenB = USDC
    // User 0 starts with all the ETH
    // User 1 starts with all the USDC
  ...
  it('should fill at maker price - market sell into bid - fill maker fully', async () => {
    // tokenA = ETH
    // tokenB = USDC
    // User 0 starts with all the ETH
    // User 1 starts with all the USDC
  ...
  it('should fill at maker price - market sell into bid - fill maker partially', async () => {
    // tokenA = ETH
    // tokenB = USDC
    // User 0 starts with all the ETH
    // User 1 starts with all the USDC
  ...
  it('should disallow self swap', async () => {
    // tokenA = ETH
    // tokenB = USDC
    // User 0 starts with all the ETH
    // User 1 starts with all the USDC

...
describe("fillOrderExactInputETH_Deposit", function () {
  ...
  it("Should revert with 'maker order not enough balance' ", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('maker order not enough balance');
  });
  ...
  it("Should revert with 'maker order not enough allowance' ", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('maker order not enough allowance');
  });
  ...
  it("Should revert when maker order is already filled", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('order is filled');
  });
  ...
  it("Should revert when maker order is canceled", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('order canceled');
  });
  ...
  it("Should revert when maker order is expired", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("1");
    const signedLeftMessage = await signOrder(TESTRPC_PRIVATE_KEYS_STRINGS[0], makerOrder, exchangeContract.address)
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('order expired');
  });
  ...
  it("should fail when filled twice", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    const tx = await exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount }))
  ...
  it("should fail without fillAvailable when over-ordering", async function () {
    ...
    await exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })
    const tx2 = exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })
  ...
  it("Should emit OrderStatus fill a order", async function () {
    ...
    expect(await exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount }))
...
describe("fillOrderExactInputETH_Withdraw", function () {
  ...
  it("Should revert with 'maker order not enough balance' ", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("1");
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('maker order not enough balance');
  ...
  it("Should revert with 'taker order not enough balance' ", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('taker order not enough balance');
  ...
  it("Should revert with 'maker order not enough allowance' ", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('maker order not enough allowance');
  ...
  it("Should revert with 'taker order not enough allowance' ", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('taker order not enough allowance');
  ...
  it("Should revert when maker order is already filled", async function () {
    ...
    const fillAmount = ethers.BigNumber.from("120");
    await exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('order is filled');
  ...
  it("Should revert when maker order is canceled", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("1");
    await exchangeContract.connect(wallets[0]).cancelOrder(Object.values(makerOrder))
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('order canceled');
  ...
  it("Should revert when maker order is expired", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("1");
    const signedLeftMessage = await signOrder(TESTRPC_PRIVATE_KEYS_STRINGS[0], makerOrder, exchangeContract.address)
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('order expired');
  ...
  it("should fail when filled twice", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    const tx = await exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false))
      .to.be.revertedWith('order is filled');
  });
  ...
  it("should fail without fillAvailable when over-ordering", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("90");
    await exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)
    const tx2 = exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)
    await expect(tx2).to.be.revertedWith('amount exceeds available size');
  });
  ...
  it("Should emit OrderStatus fill a order", async function () {
    ...
    expect(await exchangeContract.connect(wallets[1]).fillOrderExactInputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false))
      .to.emit(exchangeContract, 'OrderStatus')
      .withArgs(orderHash, ethers.utils.parseEther("100"), ethers.utils.parseEther("100"))
  });

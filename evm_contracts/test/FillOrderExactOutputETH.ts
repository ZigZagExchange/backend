...
describe("fillOrderExactOutputETH_Deposit", function () {
  ...
  beforeEach(async function () {
    ...
    exchangeContract = await Exchange.deploy("ZigZag", "2.1", FEE_ADDRESS, weth.address);

    await tokenA.mint(ethers.utils.parseEther("1000"), wallets[0].address);
    await owner.sendTransaction({ to: wallets[1].address, value: ethers.utils.parseEther("100") })
  ...
  it("Should revert with 'maker order not enough balance' ", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('maker order not enough balance');
  });
  ...
  it("Should revert with 'maker order not enough allowance' ", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('maker order not enough allowance');
  });
  ...
  it("Should revert when maker order is already filled", async function () {
    ...
    await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('order is filled');
  });
  ...
  it("Should revert when maker order is canceled", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('order canceled');
  });
  ...
  it("Should revert when maker order is expired", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("1");
    const signedLeftMessage = await signOrder(TESTRPC_PRIVATE_KEYS_STRINGS[0], makerOrder, exchangeContract.address)
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })).to.be.revertedWith('order expired');
  });
  ...
  it("should fail when filled twice", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    const tx = await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount }))
  ...
  it("should fill what's available", async function () {
    ...
    await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })
    await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, true, { value: fillAmount })
  ...
  it("should fail without fillAvailable when over-ordering", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("90");
    await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })
    const tx2 = exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount })
  ...
  it("Should emit OrderStatus fill a order", async function () {
    ...
    expect(await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false, { value: fillAmount }))
...
describe("fillOrderExactOutputETH_Withdraw", function () {
  ...
  beforeEach(async function () {
    ...
    await owner.sendTransaction({ to: wallets[0].address, value: ethers.utils.parseEther("205") })
    await weth.connect(wallets[0]).deposit({ value: ethers.utils.parseEther("205") })
  ...
  it("Should revert with 'maker order not enough balance' ", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('maker order not enough balance');

    let [owner] = await ethers.getSigners();
    await weth.connect(wallets[0]).withdraw(ethers.utils.parseEther("200"))
    await wallets[0].sendTransaction({ to: owner.address, value: ethers.utils.parseEther("200") })
  });
  ...
  it("Should revert with 'taker order not enough balance' ", async function () {
    ...
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('taker order not enough balance');
  ...
  it("Should revert with 'maker order not enough allowance' ", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('maker order not enough allowance');
  ...
  it("Should revert with 'taker order not enough allowance' ", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('taker order not enough allowance');
  ...
  it("Should revert when maker order is already filled", async function () {
    ...
    await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('order is filled');
  ...
  it("Should revert when maker order is canceled", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("1");
    await exchangeContract.connect(wallets[0]).cancelOrder(Object.values(makerOrder))
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('order canceled');
  ...
  it("Should revert when maker order is expired", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("1");
    const signedLeftMessage = await signOrder(TESTRPC_PRIVATE_KEYS_STRINGS[0], makerOrder, exchangeContract.address)
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)).to.be.revertedWith('order expired');
  ...
  it("should fail when filled twice", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    const tx = await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, true)
    await expect(exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false))
      .to.be.revertedWith('order is filled');
  });
  ...
  it("fill a full order", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("50");
    const tx = await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)
    const res = await tx.wait();

    const balance1 = await weth.balanceOf(wallets[0].address);
    const balance2 = (await provider.getBalance(wallets[1].address)).sub(balance2Before).add(res.cumulativeGasUsed.mul(res.effectiveGasPrice));
    const balance3 = await weth.balanceOf(wallets[2].address);
    const balance4 = await tokenB.balanceOf(wallets[0].address);
    const balance5 = await tokenB.balanceOf(wallets[1].address);
    const balance6 = await tokenB.balanceOf(wallets[2].address);
  ...
  it("should fill what's available", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("90");
    const balance2Before = await provider.getBalance(wallets[1].address);
    const tx1 = await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)
    const tx2 = await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, true)
  ...
  it("should fail without fillAvailable when over-ordering", async function () {
    ...
    const fillAmount = ethers.utils.parseEther("100");
    await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, true)
    const tx2 = exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false)
    await expect(tx2).to.be.revertedWith('amount exceeds available size');
  });
  ...
  it("Should emit OrderStatus fill a order", async function () {
    ...
    expect(await exchangeContract.connect(wallets[1]).fillOrderExactOutputETH(Object.values(makerOrder), signedLeftMessage, fillAmount, false))
      .to.emit(exchangeContract, 'OrderStatus')
      .withArgs(orderHash, ethers.utils.parseEther("100"), ethers.utils.parseEther("100"))
  });

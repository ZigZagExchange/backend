...
describe('FillOrderBook', () => {
  ...
  it("Should revert with 'maker order not enough balance' A", async () => {
    const makerOrderA = {
    ...
    const signedLeftMessageA = await signOrder(
      TESTRPC_PRIVATE_KEYS_STRINGS[0],
      makerOrderA,
      exchangeContract.address
    )
    const signedLeftMessageB = await signOrder(
  ...
  it("Should revert with 'maker order not enough balance' B", async () => {
    const makerOrderA = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
  ...
  it("Should revert with 'maker order not enough allowance' A", async () => {
    const makerOrderA = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
  ...
  it("Should revert with 'maker order not enough allowance' B", async () => {
    const makerOrderA = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
  ...
  it('Should not revert when only maker order A is already filled', async () => {
    const makerOrderA = {
      user: wallets[0].address,
      sellToken: tokenA.address,
      buyToken: tokenB.address,
    ...
    const fillAmountOne = ethers.utils.parseEther('150')
    await exchangeContract
      .connect(wallets[1])
      .fillOrderBook(
        [Object.values(makerOrderA), Object.values(makerOrderB)],
    ...
    expect(balance4).to.equal(ethers.utils.parseEther('150')) // 1500

    // makerB
    expect(balance3).to.equal(ethers.utils.parseEther('925')) // 1000 - 50 + 25
    expect(balance6).to.equal(ethers.utils.parseEther('150')) // 100 + 50


    expect(balance2).to.equal(ethers.utils.parseEther('175')) // 150 + 25
    expect(balance5).to.equal(ethers.utils.parseEther('700')) // 100 - (150 + 100 + 50)
  })

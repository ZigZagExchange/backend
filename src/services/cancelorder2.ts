...
export const cancelorder2: ZZServiceHandler = async (
  ...
  if (!api.VALID_CHAINS.includes(chainId)) {
    const errorMsg = { op: 'error', args: ['cancelorder2', `${chainId} is not a valid chain id. Use ${api.VALID_CHAINS}`] }
  ...
  try {
    ...
    ws.send(
      JSON.stringify({ op: 'error', args: ['cancelorder2', e.message, orderId ] })
    )
  }

...
export default function cmcRoutes(app: ZZHttpServer) {
  ...
  app.get(
    '/api/coinmarketcap/v1/markets/:chainId?',
    getChainId,
    async (req, res) => {
      try {
        const { chainId } = req
        ...
        res
          .status(400)
          .send({ op: 'error', message: 'Failed to fetch markets' })
      }
    }
  )

  app.get(
    '/api/coinmarketcap/v1/ticker/:chainId?',
    getChainId,
    async (req, res) => {
      try {
        const { chainId } = req
  ...
  app.get(
    '/api/coinmarketcap/v1/orderbook/:market_pair/:chainId?',
    getChainId,
    async (req, res) => {
      const { chainId } = req
  ...
  app.get(
    '/api/coinmarketcap/v1/trades/:market_pair/:chainId?',
    getChainId,
    async (req, res) => {
      const { chainId } = req

...
export default function cgRoutes(app: ZZHttpServer) {
  ...
  app.get(
    '/api/coingecko/v1/tickers/:chainId?',
    getChainId,
    async (req, res) => {
      try {
        const { chainId } = req
  ...
  app.get(
    '/api/coingecko/v1/orderbook/:chainId?',
    getChainId,
    async (req, res) => {
      const { chainId } = req
      ...
      try {
        let orderBook: any = await app.api.getOrderBook(
          chainId,
          market,
          depth,
          3
  ...
  app.get(
    '/api/coingecko/v1/historical_trades/:chainId?',
    getChainId,
    async (req, res) => {
      const { chainId } = req

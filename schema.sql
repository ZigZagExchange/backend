...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS active BOOLEAN;

CREATE INDEX IF NOT EXISTS idx_orders_price_insert_timestamp ON orders USING btree (price, insert_timestamp);
CREATE INDEX IF NOT EXISTS idx_orders_active ON orders USING btree (active);
...
BEGIN
  IF NEW.side = 'b' AND NEW.price >= (SELECT price FROM orders WHERE market = NEW.market AND chainid  = NEW.chainid  AND side = 's' AND unfilled > 0.0 AND active = TRUE ORDER BY price ASC LIMIT 1) THEN
    RAISE EXCEPTION 'This order would result in a crossed book.';
    RETURN NULL;
  ELSIF NEW.side = 's' AND NEW.price <= (SELECT price FROM orders WHERE market = NEW.market AND chainid  = NEW.chainid  AND side = 'b' AND unfilled > 0.0 AND active = TRUE ORDER BY price DESC LIMIT 1) THEN
    RAISE EXCEPTION 'This order would result in a crossed book.';
    RETURN NULL;
  END IF;
  RETURN NEW;
...
BEGIN
  ...
  IF _side = 'b' THEN
    FOR match IN SELECT * FROM orders WHERE market = _market AND side = 's' AND price <= _price AND active = TRUE ORDER BY price ASC, insert_timestamp ASC LOOP
      ...
      IF amount_remaining > 0 THEN
        IF amount_remaining < match.unfilled THEN
          ...
          WITH fill AS (INSERT INTO fills (chainid , market, order_id, maker_userid, taker_userid, price, amount) VALUES (_chainid , _market, match.id, match.userid, _userid, match.price, amount_taken) RETURNING id, match.price, amount_taken) INSERT INTO tmp_fills SELECT * FROM fill;
	  ...
	  IF amount_remaining = 0 THEN
        ...
        ELSE
          ...
          amount_remaining := amount_remaining - amount_taken;
          WITH fill AS (INSERT INTO fills (chainid , market, order_id, maker_userid, taker_userid, price, amount) VALUES (_chainid , _market, match.id, match.userid, _userid, match.price, amount_taken) RETURNING id, match.price, amount_taken) INSERT INTO tmp_fills SELECT * FROM fill;
          UPDATE orders SET unfilled = unfilled - amount_taken, active = FALSE WHERE id = match.id;
          IF amount_remaining = 0 THEN
            -- RAISE NOTICE '  order complete';
            EXIT; -- exit loop
          END IF;
    ...
    END LOOP;
  ELSE -- side is 's'
    FOR match IN SELECT * FROM orders WHERE chainid  = _chainid  AND market = _market AND side = 'b' AND price >= _price AND active = TRUE ORDER BY price DESC, insert_timestamp ASC LOOP
  ...
  IF amount_remaining > 0 THEN
    ...
    WITH offer AS (INSERT INTO orders (chainid , userid, market, side, price, base_quantity, active) VALUES (_chainid , _userid, _market, _side, _price, amount_remaining, TRUE) RETURNING id, side, price, base_quantity) INSERT INTO tmp_offer SELECT * FROM offer;
  END IF;
